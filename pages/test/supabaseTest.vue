...
export default {
  ...
  methods: {
    ...
    async testCloudFunction() {
      await this.safeExecuteTest('云函数测试', async () => {
        this.testing.cloudFunction = true
        this.addResult('云函数诊断', false, '开始诊断...')
        
        try {
          console.log('🔍 开始云函数环境诊断...')
          
          // 使用调试工具进行完整诊断
          const diagnosis = await cloudDebugger.diagnose()
          
          let successCount = 0
          let totalTests = diagnosis.tests.length
          
          diagnosis.tests.forEach(test => {
            if (test.success) successCount++
            this.addResult(test.name, test.success, 
              test.success ? '✅ 通过' : `❌ ${test.result.error || '失败'}`)
          })
          
          // 更新总体结果
          const overallSuccess = successCount === totalTests
          this.updateResult('云函数诊断', overallSuccess, 
            `诊断完成: ${successCount}/${totalTests} 项测试通过`)
          
          if (overallSuccess) {
            // 如果基础诊断通过，测试实际的Supabase代理
            this.addResult('Supabase代理测试', false, '测试中...')
            
            const proxyResult = await cloudDebugger.safeCallFunction('supabaseProxy', {
              action: 'healthCheck'
            })
            ...
            if (proxyResult.success) {
              ...
              this.updateResult('Supabase代理测试', false, 
                proxyResult.error || 'Supabase代理调用失败')
            }
          }
          
        } catch (error) {
          this.updateResult('云函数诊断', false, error.message)
          console.error('云函数诊断失败:', error)
        } finally {
